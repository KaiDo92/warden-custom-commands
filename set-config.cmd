...
warden db connect -e "UPDATE ${TABLE_PREFIX}core_config_data SET value = 'https://${TRAEFIK_SUBDOMAIN}.${TRAEFIK_DOMAIN}/' WHERE path IN('web/secure/base_url', 'web/unsecure/base_url', 'web/unsecure/base_link_url', 'web/secure/base_link_url')" || true
warden db connect -e "UPDATE ${TABLE_PREFIX}core_config_data SET value = 'dev_$((1000 + $RANDOM % 10000))' WHERE path = 'algoliasearch_credentials/credentials/index_prefix'" || true
...
#warden env exec -T php-fpm bin/magento app:config:dump themes scopes i18n || true
