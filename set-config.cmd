...
if [[ "$WARDEN_ELASTICSEARCH" -eq "1" ]] || [[ "$WARDEN_OPENSEARCH" -eq "1" ]]; then
    ...
    den env exec -T php-fpm bin/magento config:set --lock-env catalog/search/engine elasticsearch7 || true
    den env exec -T php-fpm bin/magento config:set --lock-env catalog/search/elasticsearch7_server_hostname $ELASTICSEARCH_HOSTNAME || true
    den env exec -T php-fpm bin/magento config:set --lock-env catalog/search/elasticsearch7_server_port 9200 || true
    den env exec -T php-fpm bin/magento config:set --lock-env catalog/search/elasticsearch7_index_prefix magento2 || true
    den env exec -T php-fpm bin/magento config:set --lock-env catalog/search/elasticsearch7_enable_auth 0 || true
    den env exec -T php-fpm bin/magento config:set --lock-env catalog/search/elasticsearch7_server_timeout 15 || true
fi
...
den env exec -T php-fpm bin/magento config:set --lock-env web/unsecure/base_url "https://${TRAEFIK_SUBDOMAIN}.${TRAEFIK_DOMAIN}/" || true
den env exec -T php-fpm bin/magento config:set --lock-env web/secure/base_url "https://${TRAEFIK_SUBDOMAIN}.${TRAEFIK_DOMAIN}/" || true
